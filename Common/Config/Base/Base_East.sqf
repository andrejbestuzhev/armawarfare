...
if (!isNil "CTI_PRICES_STRUCTURES_EAST") then {
	{
		private _cn = _x select 0;
		private _pr = CTI_PRICES_STRUCTURES_EAST getOrDefault [_cn, -1];
		if (_pr isEqualType 0 && {_pr >= 0}) then { _prices set [_forEachIndex, _pr]; };
		private _bt = CTI_PRICES_STRUCT_TIMES_EAST getOrDefault [_cn, -1];
...
if (!isNil "CTI_PRICES_DEFENSES_EAST") then {
	{
		private _pr = CTI_PRICES_DEFENSES_EAST getOrDefault [_x, -1];
		if (_pr isEqualType 0 && {_pr >= 0}) then { _prices set [_forEachIndex, _pr]; };
	} forEach _classes;
};
